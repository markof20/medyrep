...
import { pickLevelQuestions, getNodeById, getLevel, type Question } from "@/data/medContent";
import { getNodeProgress, useMedStore } from "@/lib/medStore";
import { getModuleIndexForLevel } from "@/lib/structure";
...
function LessonPage() {
  ...
  const backTo = () => {
    if (node) {
      navigate({
        ...
        params: { nodeId: node.id, moduleIndex: String(getModuleIndexForLevel(node, levelId)) },
      });
    } else {
      navigate({ to: "/" });
    }
  ...
  function handleNext() {
    if (idx + 1 >= total) {
      ...
      if (correctCount === total) addXp(20, node!.id);
      setDone(true);
      return;
    }
    setIdx((i) => i + 1);
  ...
  if (done) {
    ...
    return (
      <FinishScreen
        ...
        wrong={wrongCount}
        total={total}
        completedLevels={Math.min(node.levels.length, np.completedLevels.length)}
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 overflow-y-auto mx-auto max-w-md w-full px-4 py-4">
      ...
      {submitted && (
        <div
          className={cn(
            "sticky bottom-0 z-20 border-t-4 animate-slide-up bg-background",
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {!isCorrect() && q.type === "truefalse" && (
                  <div className="text-xs text-muted-foreground">
                    Risposta corretta:{" "}
                    <b className="text-foreground">{q.answer ? "Vero" : "Falso"}</b>
                  </div>
                )}
              </div>
            </div>
