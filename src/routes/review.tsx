      ...
      <main className="mx-auto max-w-md px-4 pt-4">
        ...
        <Tabs defaultValue="mistakes" className="w-full">
          <TabsList className="grid grid-cols-2 w-full h-12 rounded-2xl">
            <TabsTrigger value="mistakes" className="rounded-xl font-extrabold">
              I miei Errori
            </TabsTrigger>
            <TabsTrigger value="cards" className="rounded-xl font-extrabold">
              Flashcard
            </TabsTrigger>
          </TabsList>
...
function MistakesTab() {
  ...
  const items = useMemo(
    () => state.mistakes.map((id) => findQuestion(id)).filter(Boolean),
    [state.mistakes],
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between text-xs text-muted-foreground font-bold">
        <span>
          {items.length} domand{items.length === 1 ? "a" : "e"} da ripassare
        </span>
        <span>
      ...
      <div className="rounded-3xl bg-card border-2 p-5 node-shadow">
        ...
        {submitted && (
          <div
            className={cn(
              "mt-4 rounded-xl p-3 text-sm border-2",
              correct ? "bg-success/10 border-success" : "bg-destructive/10 border-destructive",
            )}
          >
            <div
              className={cn(
                "font-extrabold mb-1 flex items-center gap-1",
                correct ? "text-success" : "text-destructive",
              )}
            >
              {correct ? (
                <>
                  <Check className="size-4" /> Corretto!
                </>
              ) : (
                <>
                  <X className="size-4" /> Riprova alla prossima
                </>
              )}
            </div>
        ...
        <div className="mt-4">
          {!submitted ? (
            ...
            >
              {correct ? (
                <>
                  <Sparkles className="size-4 mr-1" /> +5 XP · Continua
                </>
              ) : (
                "Prossima"
              )}
...
function FlashcardsTab() {
  ...
  const decks = useMemo<FlashcardDeck[]>(() => {
    return NODES.filter((node) => !isReviewNode(node))
      .map((node) => {
        const progress = getNodeProgress(state, node.id);
        const completedLevelIds = new Set(progress.completedLevels);
        const cards = node.flashcards.filter((_, idx) =>
          completedLevelIds.has(node.levels[idx]?.id),
        );
  ...
  const activeDeck = decks.find((deck) => deck.node.id === nodeId) ?? decks[0];
  ...
  useEffect(() => {
    ...
    if (nodeId !== activeDeck.node.id) {
      setNodeId(activeDeck.node.id);
      setI(0);
      setFlipped(false);
    }
  }, [activeDeck, nodeId]);
  ...
  useEffect(() => {
    setI((current) => Math.min(current, Math.max(0, cards.length - 1)));
  }, [cards.length]);
  ...
  if (!activeDeck) {
    return (
      <div className="rounded-3xl border-2 border-dashed p-8 text-center">
        <div className="text-5xl mb-2">📚</div>
        <div className="font-extrabold text-lg">Nessuna flashcard sbloccata</div>
        <p className="text-sm text-muted-foreground mt-1">
          Completa almeno un livello: le flashcard appariranno solo sugli argomenti già superati.
        </p>
        <Link to="/" className="inline-block mt-4 text-primary font-extrabold">
          Vai al percorso →
        </Link>
  ...
  const currentCard = cards[Math.min(i, Math.max(0, cards.length - 1))]!;

  return (
    <div className="space-y-4">
      <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
        {decks.map((deck) => {
          const active = deck.node.id === activeDeck.node.id;
          return (
            <button
              key={deck.node.id}
              onClick={() => {
                setNodeId(deck.node.id);
                setI(0);
                setFlipped(false);
              }}
              className={cn(
            ...
            >
              {deck.node.emoji} {deck.node.title}
            </button>
          );
        })}
      </div>
      ...
      <div className="text-xs font-bold text-muted-foreground">
        {cards.length} argoment{cards.length === 1 ? "o" : "i"} superat
        {cards.length === 1 ? "o" : "i"}
      </div>
      ...
      >
        <BookOpen
          className={cn("size-6 absolute top-4 left-4", flipped ? "text-success" : "text-primary")}
        />
